...
function Result({ title, message }: { title: string; message: string }) {
  return (
    <main className="max-w-sm mx-auto pt-32 px-6 text-center">
      ...
      <a href="/" className="mt-10 inline-block text-sm text-gray-600 hover:text-gray-300 underline">
        Back to staff page
      </a>
    </main>
  )
...
export default async function ConfirmPage({
  ...
  if (data.status !== 'pending') {
    ...
    return (
      <Result
        ...
        message={`You already ${verb} your shift for ${fmtDate(data.date)}.`}
      />
    )
  }
  ...
  if (status === 'confirmed') {
    return (
      <Result
        ...
        message={`See you at Bottlerocket on ${fmtDate(data.date)}!`}
      />
    )
  }
  ...
  return (
    <Result
      ...
      message={`Got it — we'll find someone else for ${fmtDate(data.date)}.`}
    />
  )
}
